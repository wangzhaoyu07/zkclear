...
use snark_verifier::{cost::CostEstimation, verifier::plonk::PlonkVerifier, loader::evm::*};
...
pub fn run_for_debug<T: DeserializeOwned>(
    f: impl FnOnce(&mut BaseCircuitBuilder<Fr>, T, &mut Vec<AssignedValue<Fr>>),
    private_inputs: T,
) {
...
pub fn run_on_inputs<T: DeserializeOwned>(
    ...
    match cli.command {
        SnarkCmd::Mock => {
            ...
            println!("Mock time: {:?}", duration.as_secs_f64());
        }
        SnarkCmd::Keygen => {
        ...
        SnarkCmd::VerifyOnChain => {
            
            let vk_path = data_path.join(PathBuf::from(format!("{name}.vk")));
            let sol_path = data_path.join(PathBuf::from(format!("{name}.sol")));
            let pinning_path = config_path.join(PathBuf::from(format!("{name}.json")));
            let mut pinning_file = File::open(&pinning_path)
                .unwrap_or_else(|_| panic!("Could not read file at {pinning_path:?}"));
            let pinning: (BaseCircuitParams, MultiPhaseThreadBreakPoints) =
            ...
            let proof_cost = PlonkVerifier::<SHPLONK>::estimate_cost(&protocol);
            println!("Proof cost: {:?}", proof_cost);
            let gas_cost = estimate_gas(proof_cost);
