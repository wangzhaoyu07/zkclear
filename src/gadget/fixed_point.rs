...
use halo2_base::{
    gates::{GateChip, GateInstructions, RangeInstructions, RangeChip},
...
impl<F: BigPrimeField, const PRECISION_BITS: u32> FixedPointChip<F, PRECISION_BITS> {
    pub fn new(
        ...
        let negative_point = bn254_max - F::from_u128(2u128.pow(PRECISION_BITS * 2 + 1)) + F::from(1);
    ...
    pub fn default(builder: &mut BaseCircuitBuilder<F>) -> Self {
        Self::new(builder)
    }

    // Function to adjust byte slices
    fn adjust_bytes(bytes: &[u8], expected_len: usize) -> Vec<u8> {
    ...
    pub fn quantization(&self, x: f64) -> F {
        ...
        // let mut x_q_f = F::from_bytes_le(&x_q_bytes_le);
        let mut x_q_f = F::from_bytes_le(&Self::adjust_bytes(&x_q_bytes_le, FIELD_BYTE_SIZE));
        if sign < 0.0 {
            x_q_f = self.bn254_max - x_q_f + F::from(1);
        }

        x_q_f
    }

    pub fn quantization_no_rescale(&self, x: f64) -> F {
        let sign = x.signum();
        let x = x.abs();
        let x_q = (x).round() as u128;
        let x_q_biguint = BigUint::from(x_q).to_bytes_le();
        let mut x_q_bytes_le = [0u8; 64];
        for (idx, val) in x_q_biguint.iter().enumerate() {
            x_q_bytes_le[idx] = *val;
    ...
    pub fn dequantization(&self, x: F) -> f64 {
        ...
        let negative = if x > self.negative_point {
            x_mut = self.bn254_max - x - F::from(1); // Shouldn't this be +F::from(1)?
            -1f64
        } else {
            1f64
        };
    ...
    pub fn dequantization_no_rescale(&self, x: F) -> f64 {
        let mut x_mut = x;
        let negative = if x > self.negative_point {
            x_mut = self.bn254_max - x - F::from(1); // Shouldn't this be +F::from(1)?
            -1f64
...
pub trait FixedPointInstructions<F: ScalarField, const PRECISION_BITS: u32> {
    ...
    where
        F: BigPrimeField,
    {
        let a = self.gate().add(ctx, Constant(F::from(0)), a.into());
        let b = self.gate().add(ctx, Constant(F::from(0)), b.into());
        self.gate().assert_bit(ctx, a);
        self.gate().assert_bit(ctx, b);
        let ab = self.gate().add(ctx, a, b);
        let one = self.gate().add(ctx, Constant(F::from(1)), Constant(F::from(0)));
    ...
    for FixedPointChip<F, PRECISION_BITS>
{
        ...
        F: BigPrimeField,
    {
        ...
        self.gate().assert_is_const(ctx, &b_sign, &F::from(0));
        let a_abs = self.qabs(ctx, a);
    ...
    {
        let x = x.into();
        let mut intermediates = vec![Constant(F::from(0))];
        let coef_iter: Vec<QA> = coef.into_iter().collect();
        let last_idx_coef = coef_iter.len() - 1;
        let mut result: AssignedValue<F> = self.qadd(ctx, x, Constant(F::from(0)));
    ...
    {
        let a = a.into();
        let a_assigned = self.gate().add(ctx, a, Constant(F::from(0)));
        let is_neg = self.is_neg(ctx, a);
        let is_zero = self.gate().is_zero(ctx, a_assigned);
        let is_invalid = self.gate().or(ctx, is_neg, is_zero);
        self.gate().assert_is_const(ctx, &is_invalid, &F::from(0));
        ...
        let pow1_witness = self.gate().add(ctx, Witness(pow1), Constant(F::from(0)));
        let exp1 = self.gate().add(ctx, Witness(F::from(num_digits)), Constant(F::from(0)));
        self.check_power_of_two(ctx, pow1_witness, exp1);
        let pow2_witness = self.gate().mul(ctx, pow1_witness, Constant(F::from(2)));
        let exp2 = self.gate().add(ctx, exp1, Constant(F::from(1)));
        ...
        self.gate().assert_is_const(ctx, &a_bound, &F::from(1));
        ...
        let shift_pow2_witness = self.gate().add(ctx, Witness(shift_pow2), Constant(F::from(0)));
        ...
        QA: Into<QuantumCell<F>> + Copy,
    {
        ...
        assert!(a.len() == b.len());
        // using the fact that FixedPointChip's 0 is at F::from(0)
        let mut res_s = ctx.load_witness(F::from(0));
        self.gate().assert_is_const(ctx, &res_s, &F::from(0));
    ...
    fn signed_div_scale(
        ...
        let (q, r) = if a_is_neg {
            let a_abs = fe_to_biguint(&(self.bn254_max - a.value() + F::from(1)));
