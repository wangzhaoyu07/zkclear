...
fn test_matrix_to_digraph<F: BigPrimeField>(
    ...
    let num_of_graphs = args.num_sample.unwrap_or(10);
    ...
    let matrix_vec = x.adj_matrix;
    let output_dir = x.output_dir;
    let num_nodes = matrix_vec.len();
    // let mut graph_f64 = [[0.0; num_of_nodes]; num_of_nodes];
    let intervention_node_idx = 0;
    let intervention_node_idx_zk = ctx.load_witness(F::from(intervention_node_idx));
    for _ in 0..num_of_graphs {
        ...
        for i in 0..num_of_nodes {
            for j in 0..num_of_nodes {
                ...
                d_matrix[(i,j)] = matrix_vec[i][j];
                
            }
        }
        let ancestor_graph = CausalGraph::<F,PRECISION_BITS>::ancestors_identification(ctx, &fixed_point_chip, &d_matrix);
        
        let is_d_separated = CausalGraph::<F,PRECISION_BITS>::d_separation(ctx, &fixed_point_chip, &d_matrix, 5,1,vec![3], ancestor_graph);
    }
}
