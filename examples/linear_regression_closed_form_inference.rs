...
pub fn train_and_inference<F: BigPrimeField>(
    ...
    println!("train_x[0]: {:?}", train_x[0].len());

    // Initialize the chip
    const PRECISION_BITS: u32 = 63;
    const EPS_ERR: f64 = 1e-10;
    ...
    let test_x_witness: Vec<Vec<AssignedValue<F>>> = train_x_witness;
    let prediction_batch = lrchip.inference(ctx, &fpchip, test_x_witness);
}
